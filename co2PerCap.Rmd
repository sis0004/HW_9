...
TopEmissions <- as.character(co2_emissions_tonnes_per_person$country[max]) #converts factor to character
```

Your finished product might look something like this:
```{r, echo=FALSE}
dat <- rep("NA", 3)
ex_tab <- data.frame("Qatar"=dat, "United States"=dat)
colnames(ex_tab) <- c("United States", "Qatar")
